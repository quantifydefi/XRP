<template>
  <v-toolbar-items>
    <v-btn class="mt-1 subtitle-2 text-capitalize font-weight-regular" text color="primary" @click="connectToWallet()">
      <div class="white--text">
        Balance —
        <span v-if="!walletConnected" class="ml-1 subtitle-1 font-weight-medium border-bottom">{{ '$0.00' }}</span>
    ...
    <v-menu offset-y>
      <template #activator="{ on, attrs }">
        <v-btn
          class="subtitle-2 font-weight-regular text-capitalize justify-start elevation-0"
          style="border: 0"
          outlined
          color="primary"
          dark
      ...
      <v-list outlined :style="{ backgroundColor: $vuetify.theme.themes[theme].background }">
        <v-list-item
    ...
    >
      ...
      <v-card outlined :style="{ backgroundColor: $vuetify.theme.themes[theme].background }">
    ...
    <v-menu :close-on-content-click="false" :nudge-width="300" offset-y>
      <template #activator="{ on, attrs }">
        <v-btn icon target="_blank" height="24" width="24" class="mx-2 mt-1" v-bind="attrs" color="primary" v-on="on">
          <v-icon :color="$vuetify.theme.themes[theme].baseText" class="pa-0 ma-0">mdi-magnify </v-icon>
        </v-btn>
      </template>
      ...
      <v-card outlined :style="{ backgroundColor: $vuetify.theme.themes[theme].background }">
        <v-row no-gutters class="px-3 py-1">
          ...
          <v-col cols="12"> <span class="text-caption grey--text lighten-2"> Search Coins by name </span></v-col>
        </v-row>
        <v-divider class="my-1" />
        <v-row no-gutters>
          <v-col class="px-2">
